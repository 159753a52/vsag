...
search. An index that does not support this operation causes the evaluation to fail.
`--warmup-query-count` defaults to `0`; benchmark workflows that need warmup must set it explicitly.
The reference template at `tools/eval/eval_template.yaml` shows the complete YAML shape.
...
`tools/eval/run_concurrency_ab.py`. It invokes an evaluator for each variant one case at a time, so
the search measurement semantics remain those of `eval_performance`, while the
...
Prepare one shared serialized index, one evaluator built from the candidate protocol, and one
`libvsag.so` for each variant first. Both variants should normally reference that same evaluator so
only the loaded library changes. The harness uses `type: search` and does not build or modify the
index:
...
allows the evaluator realpath and hash to match, but rejects equal resolved `libvsag.so` realpaths
or hashes; there is no index-only mode. `set_immutable` defaults to `false`; when enabled, both
evaluator invocations call `Index::SetImmutable()` after deserialization and before warmup/search. Warmup defaults
...
```json
{
  ...
  "pair_number": 1,
  "outer_concurrency": 1,
  "evaluator_binary": "/path/to/candidate/eval_performance",
  "evaluator_realpath": "/path/to/candidate/eval_performance",
  "evaluator_sha256": "...",
...
missing metric, unequal paired counts, a run whose `sample != successful + error`, a sample count
below `search_query_count`, an incomplete pair, a zero baseline for a percentage metric, or fewer
than `min_valid_pairs` valid pairs is recorded in `acceptance.reasons` and fails
