...
TEST_CASE("HGraph rejects merging itself", "[ut][hgraph][merge]") {
    constexpr int64_t dim = 2;
    auto common_param = MakeCommonParam(dim);
    auto index = MakeHGraphIndex(MakeFp32HGraphJson(false), common_param);
    ...
    auto merge_result = index->Merge({vsag::MergeUnit{index, id_map}});
    REQUIRE_FALSE(merge_result.has_value());
    REQUIRE(merge_result.error().message.find("itself") != std::string::npos);
