...
TEST_CASE("SearchStatistics concurrent producers stay exact", "[ut][search_statistics]") {
    vsag::SearchStatistics stats;
    ...
    CHECK(json["dist_cmp"].GetUint64() == kThreads * kPerThread);
    CHECK(json["hops"].GetUint64() == kThreads * kPerThread);
    CHECK(json["complete"].GetBool());
}
