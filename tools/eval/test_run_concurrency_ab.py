...
if threads == __FAIL_AT__:
    ...
    def test_same_evaluator_is_allowed_with_distinct_libraries(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            spec = self.make_spec(root)
            spec["candidate"]["eval_binary"] = spec["baseline"]["eval_binary"]
            report = self.run_fake_suite(spec)

            self.assertEqual(
                report["variants"]["baseline"]["evaluator_sha256"],
                report["variants"]["candidate"]["evaluator_sha256"],
            )
            self.assertNotEqual(
    ...
    def test_incomplete_measurement_counts_invalidate_each_run(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            spec = self.make_spec(root)
            spec["acceptance"] = {"target_concurrency": [1, 2, 4], "min_valid_pairs": 1}
            spec["baseline"]["eval_binary"] = str(
                self.make_fake_evaluator(root, "baseline_incomplete", successful_count=3)
            )
            spec["candidate"]["eval_binary"] = str(
                self.make_fake_evaluator(root, "candidate_incomplete", successful_count=3)
            )
            ...
            self.assertTrue(any("baseline run" in reason and "inconsistent" in reason for reason in reasons))
            self.assertTrue(any("candidate run" in reason and "inconsistent" in reason for reason in reasons))

    def test_measurement_sample_count_must_cover_configured_queries(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            spec = self.make_spec(root)
            spec["acceptance"] = {"target_concurrency": [1, 2, 4], "min_valid_pairs": 1}
            spec["baseline"]["eval_binary"] = str(
                self.make_fake_evaluator(root, "baseline_short", sample_count=3, successful_count=3)
            )
            spec["candidate"]["eval_binary"] = str(
                self.make_fake_evaluator(root, "candidate_short", sample_count=3, successful_count=3)
            )

            report = self.run_fake_suite(spec)

            self.assertEqual(report["acceptance"]["status"], "fail")
            self.assertEqual(report["acceptance"]["minimum_measurement_sample_count"], 4)
            self.assertTrue(
                any("below configured minimum 4" in reason for reason in report["acceptance"]["reasons"])
            )
